...
function getFeatureTags(m: PixelModel): string[] {
  const tags: string[] = []
  if (m.tensor_gen) tags.push(`Tensor ${m.tensor_gen}`)
  if (m.magic_eraser) tags.push('消しゴムマジック')
      ...
      <div className="model-modal">
        ...
        <div className="model-modal__body">
          ...
          <dl className="ifd-result-card__specs">
            ...
            {model.tensor_gen && <div><dt>Tensor世代</dt><dd>{model.tensor_gen}</dd></div>}
        ...
        <div className="model-modal__footer">
          <Link prefetch={false} href={`/pixel/${model.slug}/`} className="m-btn m-btn--primary" onClick={onClose}>
