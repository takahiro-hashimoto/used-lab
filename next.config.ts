...
const nextConfig: NextConfig = {
  ...
  experimental: {
    ...
    //
    // 8 でも、広範囲を変更してキャッシュが総崩れになるコミットでは落ちた
    // （iPhone/Pixel/Galaxy の [slug] が3ブランド分の価格ログを引くため、
    //   ワーカー数 × 3 本の重いクエリがビルド開始直後に集中する）。
    // 4 に下げて同時実行を半減させる。ビルド時間は延びるが、
    // 落ちて全部やり直すより速い。
    cpus: 4,
  },
  images: {
    ...
    // どちらの環境でも同じ幅が要求される
    // 幅の段階は7つに絞ってある。Cloudflare Image Transformations は
    // 「ユニークな変換」単位の課金（無料枠 月5,000）で、実測では
    // 12段階時に約1,170変換 → 7段階で約730変換（-38%）。
    // 750/1080 は 640/828/1200 と実用上の差がなく、16/48/96 は
    // ほぼ参照されていなかった（2026-08 のページ実測より）
