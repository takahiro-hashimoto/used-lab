import type { MacModel } from '@/lib/types'
import { formatReleaseDate, getEfficiencyRating } from '@/lib/utils/macbook-helpers'

type Props = {
  model: MacModel
}

type SpecItem = {
  label: string
  value: string
  subtext: string
  icon: string
}

import RatingMark from '@/app/components/RatingMark'
import { PortSpec, DetailSpec } from '@/app/components/spec-table-utils'

function BoolValue({ value }: { value: boolean }) {
  return value ? <RatingMark mark="◯" size="sm" /> : <RatingMark mark="×" size="sm" />
}

export default function BasicSpecs({ model }: Props) {
  const releaseDate = formatReleaseDate(model.date)
  const efficiency = getEfficiencyRating(model.cpu)

  const specs: SpecItem[] = [
    {
      label: '発売日',
      value: releaseDate || '-',
      subtext: 'Appleによる発売日',
      icon: 'fa-solid fa-calendar',
    },
    {
      label: 'ディスプレイ',
      value: model.display || '-',
      subtext: 'ディスプレイの種類',
      icon: 'fa-solid fa-display',
    },
    {
      label: '輝度',
      value: model.luminance || '-',
      subtext: '画面の明るさ',
      icon: 'fa-solid fa-sun',
    },
    {
      label: '同梱物',
      value: model.included_accessories || '-',
      subtext: '本体以外に付いてくるもの',
      icon: 'fa-solid fa-keyboard',
    },
    {
      label: 'CPU',
      value: model.cpu || '-',
      subtext: '搭載チップ',
      icon: 'fa-solid fa-microchip',
    },
    {
      label: '電力効率',
      value: efficiency,
      subtext: 'チップ世代から推定',
      icon: 'fa-solid fa-leaf',
    },
  ]

  return (
    <section className="l-section" id="specs" aria-labelledby="heading-specs">
      <div className="l-container">
        <h2 className="m-section-heading m-section-heading--lg" id="heading-specs">
          {model.model}の基本スペック
        </h2>
        <p className="m-section-desc">
          {model.model}の主要スペックを一覧でまとめました。
        </p>

        {/* テキスト系スペック（6枚カード） */}
        <div className="l-grid l-grid--3col l-grid--gap-lg l-grid--mb-2xl">
          {specs.map((spec) => (
            <div key={spec.label} className="m-card m-card--sm m-stat-card">
              <p className="m-stat-card__label">
                <i className={spec.icon} aria-hidden="true"></i> {spec.label}
              </p>
              <p className="m-stat-card__value">{spec.value}</p>
              <p className="m-stat-card__note">{spec.subtext}</p>
            </div>
          ))}
        </div>

        {/* 詳細テーブル */}
        <div className="m-card m-card--shadow m-table-card">
          <table className="m-table">
            <caption className="visually-hidden">{model.model}の詳細スペック一覧</caption>
            <thead>
              <tr>
                <th scope="col">項目</th>
                <th scope="col">仕様</th>
                <th scope="col">説明</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td><i className="fa-solid fa-display" aria-hidden="true"></i> 内蔵ディスプレイ</td>
                <td><BoolValue value={model.display_builtin} /></td>
                <td>画面が本体に内蔵されているか（iMacのみ）</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-memory" aria-hidden="true"></i> メモリ</td>
                <td>{model.ram || '-'}</td>
                <td>搭載メモリ容量（ユニファイドメモリ）</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-hard-drive" aria-hidden="true"></i> ストレージ</td>
                <td>{model.strage || '-'}</td>
                <td>内蔵SSD容量</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-palette" aria-hidden="true"></i> GPUコア</td>
                <td>{model.gpu || '-'}</td>
                <td>グラフィック処理のコア数</td>
              </tr>
              {/* ポートは種類ごとに行を分け、値は「何基あるか」だけにする。
                  空欄は未入力ではなく非搭載なので ✕ を出す */}
              <tr>
                <td><i className="fa-solid fa-bolt" aria-hidden="true"></i> Thunderbolt</td>
                <td><PortSpec value={model.thunderbolt} /></td>
                <td>Thunderbolt端子の本数</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-bolt" aria-hidden="true"></i> Thunderboltの規格</td>
                <td><DetailSpec value={model.thunderbolt_gen} /></td>
                <td>転送速度を決める世代</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-plug" aria-hidden="true"></i> USB-C</td>
                <td><PortSpec value={model.usb_c} /></td>
                <td>Thunderbolt以外のUSB-C端子の本数</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-plug" aria-hidden="true"></i> USB-A</td>
                <td><PortSpec value={model.usb_a} /></td>
                <td>従来の四角いUSB機器を変換なしで挿せるか</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-sd-card" aria-hidden="true"></i> SDカードスロット</td>
                <td><BoolValue value={model.slot} /></td>
                <td>SDXCカードスロットの有無</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-tv" aria-hidden="true"></i> HDMIポート</td>
                <td><BoolValue value={model.hdmi} /></td>
                <td>外部ディスプレイとの直接接続</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-headphones" aria-hidden="true"></i> ヘッドフォンジャック</td>
                <td><BoolValue value={model.headphone} /></td>
                <td>3.5mmステレオミニ端子の有無</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-desktop" aria-hidden="true"></i> 外部ディスプレイ</td>
                <td><DetailSpec value={model.external_display} /></td>
                <td>外部モニターの同時接続可能台数</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-volume-high" aria-hidden="true"></i> オーディオ</td>
                <td>{model.speaker || '-'}</td>
                <td>搭載スピーカーの仕様</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-network-wired" aria-hidden="true"></i> Ethernet</td>
                <td><DetailSpec value={model.ethernet} /></td>
                <td>有線LANの規格</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-keyboard" aria-hidden="true"></i> 同梱物</td>
                <td>{model.included_accessories || '-'}</td>
                <td>キーボード・マウスが付属するか</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-video" aria-hidden="true"></i> カメラ</td>
                <td>{model.camera || '-'}</td>
                <td>内蔵カメラの解像度</td>
              </tr>
              <tr>
                <td><i className="fa-solid fa-expand" aria-hidden="true"></i> 解像度</td>
                <td>{model.resolution || '-'}</td>
                <td>ディスプレイの画素数</td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>
    </section>
  )
}
