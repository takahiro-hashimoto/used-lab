  ...
  },
  {
    ...
    answer: (
      <>
        ...
        <p>
          ただし、セルラーモデルはSIMカードの契約が別途必要です。中古の場合はSIMロックの有無やネットワーク利用制限の確認も必要になります。詳しくは「<a href="/ipad/wifi-cellular/">Wi-FiモデルとCellularモデルの違い</a>」で解説しています。
        </p>
      </>
    ),
  },
...
export const postCheckItems: CheckItem[] = [
  {
    ...
    text: (
      <p>
        初期設定時に前の所有者のApple IDを求められたら要注意。<strong>ロック未解除の端末は使用不可。</strong>「iPadを探す」がオフか確認し、ロックがあればすぐショップへ連絡。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        白・黒・赤の画面を表示し、ドット抜け・色ムラ・焼き付きを確認。iPad Proの有機EL（タンデムOLED）モデルは焼き付きに特に注意。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        「設定」→「Touch IDとパスコード」（またはFace ID）から新規登録を試し、正常に認識されるか確認。非正規修理品では動作しない場合あり。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        ケーブルを挿して正常に充電されるか確認。接触不良で途切れる端末もあり。USB-Cモデルは外部ディスプレイ出力もテスト推奨。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        前面・背面カメラのフォーカスを確認。スピーカーは動画再生で音割れ・左右差をチェック。4スピーカーモデルは全方向の音出しを確認。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        ペアリングと充電が正常か確認。第2世代・Proはマグネット吸着の強さもチェック。筆圧感知や反応の遅延もテスト。
      </p>
    ),
  },
]
...
export const failurePatterns: FailurePattern[] = [
  {
    ...
    text: (
      <p>
        手持ちのApple Pencilに非対応だったパターン。<strong>Apple Pencilは世代ごとに対応モデルが異なります。</strong>購入前に必ず対応表を確認。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        安さで32GBを選び、アプリ数個で容量不足に。iPadはSDカード増設不可のため<strong>容量不足は致命的。</strong>動画・ゲーム用途なら最低64GB、クリエイティブなら128GB以上を。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        古いモデルを安く買い、すぐにiPadOSサポートが終了。アプリが使えなくなり買い替えに。<strong>「あと何年使えるか」</strong>を必ず確認。
      </p>
    ),
  },
  {
    ...
    text: (
      <p>
        カフェや移動中にネット接続できず不便だったパターン。テザリングで対処可能だが手間がかかる。外出利用が多い方はセルラーモデルを検討。
      </p>
    ),
  },
]
