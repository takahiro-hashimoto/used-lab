...
type FilterType = 'all' | 'pro' | 'standard' | 'a'

function getModelCategory(model: string): string {
  const lower = model.toLowerCase()
  if (lower.includes('pro')) return 'pro'
  if (/a$/.test(model.trim())) return 'a'
  return 'standard'
}

const PIXEL_STORAGE_STEPS = [128, 256, 512, 1024]
      ...
      <div className="l-container">
          ...
          <div className="m-card m-card--shadow m-table-card">
            <div className="m-table-scroll">
              <table className="m-table battery-table">
                <caption className="visually-hidden">歴代Google Pixelストレージ容量・中古相場一覧表</caption>
                <thead>
                  <tr>
                    ...
                    <tr key={m.id}>
                      ...
                      <td>
                        <div className="storage-tags">
                          {expandStorageRange(m.strage, PIXEL_STORAGE_STEPS).map((opt) => (
        ...
        <div className="m-callout m-callout--tip u-mt-2xl">
          ...
          <p className="m-callout__text">
            ...
            詳しい価格推移は「<Link prefetch={false} href="/pixel/price-info/">中古Pixel価格相場グラフ</Link>」で確認できます。
          </p>
        </div>
      </div>
    </section>
