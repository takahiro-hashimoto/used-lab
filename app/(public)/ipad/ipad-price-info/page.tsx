...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = `中古iPad${modelCount}機種の相場・値段を毎日更新。iPad mini・Air・Pro・第9世代など歴代モデルの価格推移グラフ・相場一覧を掲載。`
...
export default async function IPadPriceInfoPage() {
  ...
  const webAppJsonLd = buildWebApplicationJsonLd({
    ...
    description: `中古iPad${modelCount}機種の価格相場を毎日更新。価格推移グラフ、相場ランキングを掲載。`,
    url: PAGE_URL,
    modelCount,
