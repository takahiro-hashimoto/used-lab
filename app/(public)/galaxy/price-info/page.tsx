...
import { CHART_COLORS, FAQ_ITEMS } from '@/lib/data/galaxy-price-info'
...
export type ModelData = {
  ...
  iosysUrl: string
  featureTags: string[]
  /** シリーズグループ（'S Ultra' | 'S' | 'Z Fold' | 'Z Flip' | 'A'）。チップの data-series・並び替えに使用 */
  series: string
  prices: PriceEntry[]
...
function getCameraLabel(model: GalaxyModel): string {
  if (model.tele_camera) return '広角 + 超広角 + 望遠'
  if (model.ultrawide_camera) return '広角 + 超広角'
  return '広角のみ'
}

// filter-searchのカードと同じ特徴タグ（Galaxy向け）
function getFeatureTags(model: GalaxyModel): string[] {
  const tags: string[] = []
  if (model.galaxy_ai) tags.push('Galaxy AI')
  if (model.s_pen) tags.push('S Pen')
  if (model.circle_to_search) tags.push('かこって検索')
  if (model.dex) tags.push('DeX')
  return tags.slice(0, 4)
}

function calcAvgPriceFromLog(log: GalaxyPriceLog): PriceEntry | null {
...
// Galaxyのシリーズグループ: Ultraは名前で判定し 'S Ultra' として別グループに
function getModelSeries(model: GalaxyModel): string {
  if (model.model.includes('Ultra')) return 'S Ultra'
  return model.series || 'S'
}

function seriesOrder(series: string): number {
  const order: Record<string, number> = { 'S Ultra': 1, S: 2, 'Z Fold': 3, 'Z Flip': 4, A: 5 }
  return order[series] ?? 6
}

// ============================================================
// ISR: 自動再生成しない（価格更新は価格ログ側の tag 無効化で反映）
// ============================================================
...
const PAGE_URL = 'https://used-lab.jp/galaxy/price-info/'

const getModels = cache(getAllGalaxyModelsIncludingEnded)

export async function generateMetadata(): Promise<Metadata> {
  const allModels = await getModels()
  const modelCount = allModels.length
  const title = buildPriceInfoTitle('Samsung Galaxy', modelCount, PRICE_INFO_UPDATE_MONTH)
  const description = `中古Samsung Galaxy${modelCount}機種の相場・値段を毎日更新。Galaxy S・A・Z折りたたみなど歴代モデルの価格推移グラフ・相場一覧を掲載。`
  return buildPriceInfoMetadata({ title, description, canonicalPath: '/galaxy/price-info/', heroImageUrl: getHeroImage('/galaxy/price-info/') })
}
...
export default async function GalaxyPriceInfoPage() {
  ...
  for (const m of modelsData) {
    if (seriesGroups[m.series]) seriesGroups[m.series].push(m.id)
  }
  ...
  const sortedModels = [...modelsData].sort((a, b) => {
    const aOrder = seriesOrder(a.series)
    const bOrder = seriesOrder(b.series)
    if (aOrder !== bOrder) return aOrder - bOrder
    return parseInt(b.year) - parseInt(a.year)
  })

  // ヒーロー・リード文の機種数は全モデル数（価格ログが空でも実数を表示）
  const modelCount = allModels.length
  const dataModelCount = modelsData.length
  const cheapestModel = rankingData[0]

  const { dateStr, dateDisplay, dateModified } = buildPageDates(modelsData)

  // JSON-LD
  const breadcrumbJsonLd = buildBreadcrumbJsonLd('中古Samsung Galaxyおすすめ機種・選び方まとめ', 'https://used-lab.jp/galaxy/', 'Samsung Galaxyの中古相場一覧')
  const webAppJsonLd = buildWebApplicationJsonLd({
    name: '中古Samsung Galaxy価格比較ダッシュボード',
    description: `中古Samsung Galaxy${dataModelCount}機種の価格相場を毎日更新。価格推移グラフ、スペック比較、相場ランキングを掲載。`,
    url: PAGE_URL,
          ...
          <div className="hero-inner l-container">
            <div className="hero-content">
              <h1 className="hero-title" itemProp="headline">
                中古Samsung Galaxyの相場・値段一覧【{PRICE_INFO_UPDATE_MONTH}】歴代{modelCount}機種の価格推移を毎日更新
              </h1>
            ...
            <div className="hero-visual">
              <figure className="hero-media">
                <Image
                  src={getHeroImage('/galaxy/price-info/')}
                  alt="中古Samsung Galaxy価格相場"
        ...
        <section className="l-section l-section--sm section-lead" aria-label="記事の導入">
          <div className="l-container">
            <div className="lead-box">
              <p>中古Samsung Galaxy<strong>{modelCount}機種</strong>の価格相場を毎日更新でお届け。</p>
              <p>
...
<p className="toc-title"><i className="fa-solid fa-list" aria-hidden="true"></i> タップできる目次</p>
        ...
        <div className="l-sections" itemProp="articleBody">
          ...
          <section className="l-section l-section--sm" id="pd-method">
            <div className="l-container">
              <h2 className="m-section-heading m-section-heading--lg">中古Galaxyの相場価格 算出方法</h2>
              ...
              <div className="l-grid l-grid--3col l-grid--gap-lg">
                <div className="m-card m-card--shadow m-card--padded post-check-item">
                  ...
                  <div className="media-card__desc m-rich-text">
                    ...
                    <p>中古Galaxyの取引量が多い主要3ショップを対象とすることで、市場全体の価格動向を反映しています。</p>
                ...
                <div className="m-card m-card--shadow m-card--padded post-check-item">
                  ...
                  <div className="media-card__desc m-rich-text">
                    <p>機種ごとの最小容量モデル（例：Galaxy S24は256GB）を対象としています。</p>
              ...
              <div className="l-container">
                <div className="m-callout m-callout--muted">
                  ...
                  <p className="m-callout__text">
                    現在価格データを集計中です。毎日更新予定です。価格推移グラフ・値下がりランキング・価格推移データは、データがそろい次第このページに掲載します。
                  </p>
                </div>
              </div>
            </section>
    ...
    <GalaxyArticleFooter
            pageUrl={PAGE_URL}
            pageTitle={`Samsung Galaxyの中古相場一覧 | 歴代${modelCount}機種の価格推移を独自集計`}
            excludeHref={["/galaxy/price-info/"]}
          />
    </>
  )
}
