...
import RecommendBanner from '@/app/watch/[slug]/components/RecommendBanner'
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: '/images/watch/watch-10.jpg', width: 360, height: 360, alt: title }],
    },
    twitter: {
      title,
      description,
...
export default async function WatchPriceInfoPage() {
  ...
  const allPriceLogs = await Promise.all(
    allModels.map((m) => getWatchPriceLogsByModelId(m.id))
  )
  ...
  return (
    <main>
      <article itemScope itemType="https://schema.org/Article">
        ...
        <ShareBox url={PAGE_URL} text={`Apple Watchの中古相場一覧 | 歴代${modelCount}機種の価格推移を独自集計`} bgSubtle />
      </article>
    </main>
  )
}
