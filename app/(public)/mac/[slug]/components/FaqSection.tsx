...
import type { MacModel, MacPriceLog, ProductShopLink } from '@/lib/types'
import {
  ...
  formatReleaseDate,
} from '@/lib/utils/macbook-helpers'
...
function buildDisplayFaqs(
  ...
  faqs.push({
    ...
    answer: (
      <>
        <p>中古{model.model}の購入先としては下記が考えられます。おすすめは中古パソコン専門店での購入。購入から一定期間内にトラブルがあった際にショップ独自の保証を受けることができるからです。</p>
        <ul>
          <li>中古パソコン専門店</li>
          <li>総合リユースショップ</li>
  ...
  faqs.push({
    ...
    answer: (
      <>
        <p>
          イオシスがおすすめです。中古Macの価格が他のサイトに比べて安い場合が多く、お得にMacBookを購入することができるからです。購入後の保証期間も長く、独自の保証延長サービスがあるのもおすすめのポイント。
        </p>
        {iosysLink && (
          <p>
  ...
  faqs.push({
    ...
    answer: osLife.isSupported ? (
      ...
      <p>
        {model.model}のmacOSアップデートサポートは終了している可能性があります。
