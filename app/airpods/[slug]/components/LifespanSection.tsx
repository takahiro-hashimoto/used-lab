...
import { calculateFirmwareLifespan, calculateRepairLifespan } from '@/lib/utils/airpods-helpers'
...
export default function LifespanSection({ model }: Props) {
  ...
  if (fwLife.releaseYear === 0) return null
    ...
    <section className="l-section l-section--bg-subtle" id="lifespan" aria-labelledby="heading-lifespan">
      <div className="l-container">
        <h2 className="m-section-heading m-section-heading--lg" id="heading-lifespan">
          {model.name}（{model.model}）はいつまで使える？製品寿命は？
        </h2>
        <p className="m-section-desc">
          ファームウェアサポートと修理受付の2つの期間から{model.name}の寿命目安を計算しました。
        </p>
        ...
        <div className="l-grid l-grid--2col l-grid--gap-lg l-grid--mb-xl">
          <div className="m-card m-stat-card m-stat-card--lg lifespan-card">
            <span className="m-badge m-badge--primary lifespan-card-label">
              <i className="fa-solid fa-code-branch" aria-hidden="true"></i> ファームウェアサポート
            </span>
            <p className="m-stat-card__value">
              {fwLife.isSupported ? (
                <><time dateTime={`${fwLife.endYear}-09`}>{fwLife.endYear}年頃</time>まで</>
              ) : (
                <>終了済み</>
              )}
            </p>
            <p className="m-stat-card__note">
              最新のファームウェア機能が使える目安。発売から7年ほどが経過するとサポート終了になる傾向がある。
            </p>
          </div>
          <div className="m-card m-stat-card m-stat-card--lg lifespan-card">
            <span className="m-badge m-badge--primary lifespan-card-label">
            ...
            <p className="m-stat-card__value">
              {repairLife.isSupported ? (
                <><time dateTime={`${repairLife.repairEndYear}-09`}>{repairLife.repairEndYear}年頃</time>まで</>
              ) : (
                <>終了済み</>
              )}
            </p>
            <p className="m-stat-card__note">
              Apple正規店でのバッテリー交換や修理が可能な期間の目安。
            </p>
          </div>
        </div>
          ...
          <dl className="glossary-list">
            <div className="glossary-item">
              ...
              <dd className="glossary-item-desc">
                発売から7年前後が経過するとファームウェアアップデートが終了する傾向がある。古いファームウェアのAirPodsを使い続けると、接続先デバイスとの互換性が低下したり、新機能が利用できなくなるリスクがある。
              </dd>
            </div>
            <div className="glossary-item">
              <dt className="glossary-item-title">ビンテージ製品</dt>
              <dd className="glossary-item-desc">
                販売終了から5年以上7年未満が経過したAirPodsが対象。修理に必要なパーツがない場合に修理サポートが受けられなくなる。
              </dd>
            </div>
            <div className="glossary-item">
              <dt className="glossary-item-title">オブソリート製品</dt>
              <dd className="glossary-item-desc">
                販売終了から7年以上が経ったAirPodsが対象。Appleの正規修理サポートを受けることが一切できなくなる。
              </dd>
            </div>
          </dl>
        </aside>
