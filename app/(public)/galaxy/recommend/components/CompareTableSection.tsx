...
import { RECOMMEND_COUNT_LABEL } from '@/lib/data/galaxy-recommend'
    ...
    <section className="l-section" id="compare" aria-labelledby="heading-compare">
      <div className="l-container">
        <h2 className="m-section-heading m-section-heading--lg" id="heading-compare">
          {heading ?? <>おすすめGalaxy{RECOMMEND_COUNT_LABEL}のスペック比較表</>}
        </h2>
        {descriptions ? (
        ...
        <div className="m-card m-card--shadow m-table-card">
          <div className="m-table-scroll">
            <table className="m-table m-table--center">
              <caption className="visually-hidden">おすすめ中古Galaxy{RECOMMEND_COUNT_LABEL}のスペック比較表</caption>
              <thead>
                <tr>
                  <th scope="col">項目</th>
                  {items.map(({ model }) => (
              ...
              <tbody>
                ...
                <tr>
                  ...
                  {items.map(({ model }) => (
                    <td key={model.id}>{model.series ? `${model.series}シリーズ` : '-'}</td>
                  ))}
                </tr>
                ...
                <tr>
                  ...
                  {items.map(({ model }) => {
                    const s = getGalaxySupport(model)
                    return (
                      <td key={model.id}>
                        {s.endLabel ? (
                          <>
                            ...
                            {s.remainingYears != null && (
                              <>
                                <br />
                                <small>（残り約{s.remainingYears}年）</small>
                              </>
                            )}
                          </>
                        ) : '-'}
