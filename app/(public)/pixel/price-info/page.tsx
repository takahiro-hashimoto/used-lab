...
import { CHART_COLORS, FAQ_ITEMS } from '@/lib/data/pixel-price-info'
import Breadcrumb from '@/app/components/Breadcrumb'
import dynamic from 'next/dynamic'

// 価格データが入った際に表示するデータセクションは Pixel 専用コンポーネント。
// 現状 pixel_price_logs は空で、これらは空データガードにより描画されない。
...
function getCameraLabel(model: PixelModel): string {
  if (model.tele_camera) return '広角 + 超広角 + 望遠'
  if (model.ultrawide_camera) return '広角 + 超広角'
  return '広角のみ'
}

// Pixel 固有機能から特徴タグを最大4つ生成
function getFeatureTags(model: PixelModel): string[] {
  const tags: string[] = []
  if (model.magic_eraser) tags.push('消しゴムマジック')
  if (model.best_take) tags.push('ベストテイク')
  if (model.video_boost) tags.push('動画ブースト')
  if (model.temp_sensor) tags.push('温度センサー')
  if (model.magic_editor) tags.push('編集マジック')
  if (model.night_sight) tags.push('夜景モード')
  if (model.face_unlock) tags.push('顔認証')
  return tags.slice(0, 4)
}

function calcAvgPriceFromLog(log: PixelPriceLog): PriceEntry | null {
...
function getModelSeries(name: string): string {
  if (name.includes('Pro XL')) return 'Pro XL'
  if (name.includes('Pro')) return 'Pro'
  if (/a$/.test(name.trim())) return 'a'
  return 'Standard'
}

function seriesOrder(series: string): number {
  const order: Record<string, number> = { 'Pro XL': 1, Pro: 2, Standard: 3, a: 4 }
  return order[series] ?? 5
}

// ============================================================
// ISR: 時間ベースの自動再生成なし（更新は purgeTag で明示無効化）
// ============================================================
...
const PAGE_URL = 'https://used-lab.jp/pixel/price-info/'

const getModels = cache(getAllPixelModelsIncludingEnded)

export async function generateMetadata(): Promise<Metadata> {
  const allModels = await getModels()
  const modelCount = allModels.length
  const title = buildPriceInfoTitle('Google Pixel', modelCount, PRICE_INFO_UPDATE_MONTH)
  const description = `中古Google Pixel${modelCount}機種の相場・値段を毎日更新。Pixel 9・8a・Proなど歴代モデルの価格推移グラフ・相場一覧を掲載。`
  return buildPriceInfoMetadata({ title, description, canonicalPath: '/pixel/price-info/', heroImageUrl: getHeroImage('/pixel/price-info/') })
}
...
export default async function PixelPriceInfoPage() {
  ...
  for (let i = 0; i < allModels.length; i++) {
    ...
    const supportUntil = model.support_until ? `${model.support_until.split('-')[0]}年頃` : '-'
    ...
    modelsData.push({
      ...
      supportUntil,
      supportEnded: model.last_android !== null,
  ...
  const hasData = modelsData.length > 0
  const modelCount = modelsData.length

  const { dateStr, dateDisplay, dateModified } = buildPageDates(modelsData)

  // FAQ / Breadcrumb は常時描画。データ依存の WebApplication / Dataset は hasData 時のみ。
  const faqJsonLd = buildFaqJsonLd(FAQ_ITEMS)
  const breadcrumbJsonLd = buildBreadcrumbJsonLd(
    '中古Google Pixelおすすめ機種・選び方まとめ',
    'https://used-lab.jp/pixel/',
    'Google Pixelの中古相場一覧',
  )

  // データ依存の派生値（hasData 時のみ利用）
  const rankingData = hasData ? buildRankingData(modelsData) : []
  const priceDropRanking = hasData ? buildPriceDropRanking(modelsData) : []
  ...
  const webAppJsonLd = hasData
    ? buildWebApplicationJsonLd({
        name: '中古Google Pixel価格比較ダッシュボード',
        description: `中古Google Pixel${modelCount}機種の価格相場を毎日更新。価格推移グラフ、スペック比較、相場ランキングを掲載。`,
        url: PAGE_URL,
        modelCount,
    ...
    : null
  const datasetJsonLd = hasData
    ? buildDatasetJsonLd({
        productLabel: '中古Google Pixel',
        url: PAGE_URL,
        modelCount,
        lowestPrice: cheapestModel?.currentPrice ?? 0,
        highestPrice: rankingData[rankingData.length - 1]?.currentPrice ?? 0,
  ...
  const heroTitle = hasData
    ? `中古Google Pixelの相場・値段一覧【${PRICE_INFO_UPDATE_MONTH}】歴代${modelCount}機種の価格推移を毎日更新`
    : `中古Google Pixelの相場・値段一覧【${PRICE_INFO_UPDATE_MONTH}】価格データを集計中`
          ...
          <div className="hero-inner l-container">
            ...
            <div className="hero-visual">
              <figure className="hero-media">
                <Image
                  src={getHeroImage('/pixel/price-info/')}
                  alt="中古Google Pixel価格相場"
        ...
        <section className="l-section l-section--sm section-lead" aria-label="記事の導入">
          <div className="l-container">
            <div className="lead-box">
              {hasData ? (
                <p>中古Google Pixel<strong>{modelCount}機種</strong>の価格相場を毎日更新でお届け。</p>
              ) : (
                <p>中古Google Pixelの価格相場を毎日更新でお届けする予定です。</p>
              )}
              <p>
...
<p className="toc-title"><i className="fa-solid fa-list" aria-hidden="true"></i> タップできる目次</p>
        ...
        <div className="l-sections" itemProp="articleBody">
          ...
          <section className="l-section l-section--sm" id="pd-method">
            <div className="l-container">
              <h2 className="m-section-heading m-section-heading--lg">中古Google Pixelの相場価格 算出方法</h2>
              ...
              <div className="l-grid l-grid--3col l-grid--gap-lg">
                <div className="m-card m-card--shadow m-card--padded post-check-item">
                  ...
                  <div className="media-card__desc m-rich-text">
                    ...
                    <p>中古Google Pixelの取引量が多い主要3ショップを対象とすることで、市場全体の価格動向を反映しています。</p>
                ...
                <div className="m-card m-card--shadow m-card--padded post-check-item">
                  ...
                  <div className="media-card__desc m-rich-text">
                    <p>機種ごとの最小容量モデル（例：Pixel 8は128GB）を対象としています。</p>
              ...
              <div className="l-container">
                <div className="m-callout m-callout--muted">
                  ...
                  <p className="m-callout__text">
                    現在価格データを集計中です。毎日更新予定です。中古Google Pixelの価格推移グラフ・最安値ランキング・値下がりランキングは、データが揃い次第このページに掲載します。
                  </p>
                </div>
              </div>
            </section>
          ...
          <section className="l-section" id="pd-faq" aria-labelledby="pd-faq-title">
            <div className="l-container">
              ...
              <p className="m-section-desc">中古Google Pixel購入に関するよくある疑問にお答えします。</p>
    ...
    <PixelArticleFooter
            pageUrl={PAGE_URL}
            pageTitle={`Google Pixelの中古相場一覧 | 歴代${modelCount}機種の価格推移を独自集計`}
            excludeHref={["/pixel/price-info/"]}
          />
    </>
  )
}
