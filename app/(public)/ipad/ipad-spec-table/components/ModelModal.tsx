...
function getFeatureTags(m: IPadModel): string[] {
  const tags: string[] = []
...
export default function ModelModal({ model, avgPrice, iosysUrl, onClose }: Props) {
  const supported = model.last_ipados === null
            ...
            <div>
              {supported ? (
                <span className="ifd-tag ifd-tag--supported">
                  <i className="fa-solid fa-shield-halved" aria-hidden="true" /> OSサポート {estimateSupportEnd(model.date, model.last_ipados)}
                </span>
              ) : (
