  ...
  },
  {
    ...
    answer: (
      <>
        ...
        <p>
          ただし、<strong>赤ロム永久保証付きの中古ショップで購入すれば、万が一×になっても交換・返金してもらえるためリスクはほぼありません。</strong>相場より安く買えるメリットもあるので、コストを抑えたい方は検討する価値があります。詳しくは<Link prefetch={false} href="/iphone/network-limit/">こちらの記事</Link>で解説しています。
        </p>
      </>
    ),
  },
...
export const postCheckItems: CheckItem[] = [
  {
    ...
    text: (
      <>
        <p>
          初期設定時に前の所有者のApple IDを求められたら要注意。<strong>アクティベーションロックが解除されていない端末は使用できません。</strong>
        </p>
        <p>
          「iPhoneを探す」がオフになっているか確認しましょう。ロックがかかっていた場合はすぐにショップへ連絡してください。
        </p>
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <p>
          購入時に「◯」でも、届いた後に再度確認しましょう。「設定」→「一般」→「情報」からIMEIをメモし、各キャリアの確認サイトで入力すれば現在の状態がわかります。
        </p>
        <p>
          「△」や「×」になっていたら返品対象です。
        </p>
  ...
  },
  {
    ...
    text: (
      <>
        <p>
          「設定」→「バッテリー」→「バッテリーの状態と充電」から確認。商品ページの記載と一致しているかをチェックしてください。
        </p>
        <p>
          記載より大幅に低い場合（5%以上の差など）は、返品・交換の対象になる可能性があります。
        </p>
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <p>
          生体認証が正常に動作するか確認。非正規修理品では動作しないことがあります。
        </p>
        <p>
          「設定」→「Face IDとパスコード」から新規登録を試し、正常に認識されるかテストしてください。
        </p>
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <p>
          前面・背面カメラで写真・動画を撮影し、フォーカス・手ブレ補正が正常に動くかを確認。
        </p>
        <p>
  ...
  },
  {
    ...
    text: (
      <>
        <p>
          「設定」→「画面表示と明るさ」→「True Tone」がオン/オフ切り替えできるか確認。非正規ディスプレイ交換品では機能しないことがあります。
        </p>
        <p>
          グレーアウトして操作できない場合は、画面が交換されている可能性があります。
        </p>
      </>
    ),
  },
...
export const failurePatterns: FailurePattern[] = [
  ...
  },
  {
    ...
    text: (
      <>
        <p>
          外装ランクだけを見て購入し、バッテリー最大容量を確認しなかったパターン。「美品」でもバッテリーが70%台ということも。
        </p>
        <p>
  ...
  },
  {
    ...
    text: (
      <>
        <p>
          古い機種を安く買ったものの、すぐにiOSサポートが終了したパターン。アプリが使えなくなったり、セキュリティリスクが高まったりして、結局買い替えが必要に。
        </p>
        <p>
          <strong>「あと何年使えるか」</strong>を考えずに購入すると損をします。
        </p>
      </>
    ),
  },
