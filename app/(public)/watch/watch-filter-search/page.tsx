        ...
        <section className="l-section" aria-labelledby="heading-logic">
          <div className="l-container">
            ...
            <div className="m-card m-card--shadow m-card--padded">
              ...
              {filterSearchNoteParagraphs('watch').map((text, i) => (
                <p key={i} style={{ marginTop: i === 0 ? undefined : '12px' }}>
                  {text.startsWith('※') ? <small>{text}</small> : text}
                </p>
              ))}
              <p style={{ marginTop: '12px' }}>価格推移は<Link prefetch={false} href="/watch/watch-price-info/">歴代Apple Watchの中古相場と価格推移</Link>でご覧いただけます。</p>
            </div>
          </div>
        </section>
